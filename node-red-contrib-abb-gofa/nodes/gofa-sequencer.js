...
module.exports = function(RED) {
    function GoFaSequencerNode(config) {
        ...
        node.on('input', function(msg, send, done) {
            ...
            if (!node.steps || !node.steps.length) { node.warn('No steps configured'); return done(); }

            var cmds = [];
            for (var i = 0; i < node.steps.length; i++) {
                var pt = r.findPoint(node.steps[i].name);
                if (!pt) { node.warn('Point not found: ' + node.steps[i].name); continue; }
            ...
            function runStep(idx) {
                ...
                r.socketSend(c.token).then(function(ack) {
                    ...
                    send([stepMsg, null]);
                    setTimeout(function() { runStep(idx + 1); }, node.dwell);
                }).catch(function(err) {
